...
个人主页包含以下内容：
\begin{itemize}
    \item \textbf{个人简介}：包含姓名（田天威）、学校与专业方向（USTC | C/C++ | Python | 信息与计算科学）
    \item \textbf{关于我}：介绍个人背景、兴趣爱好及学习方向
    \item \textbf{研究方向与兴趣}：涵盖信息与计算科学、计算机图形学、人工智能等领域
    \item \textbf{学习笔记}：提供微分几何学习笔记的 PDF 下载
...
博客中包含了详细的数学公式推导（如光线参数方程 \(P(t) = O + t \cdot d\)、球体求交的二次方程、反射与折射公式等），并配有扫描线算法和 BVH 加速结构的示意图。
...
\begin{enumerate}
    ...
    \item \textbf{内容填充}：根据"主页内容.txt"中的信息填充个人资料，包括姓名、学校、兴趣方向等
    \item \textbf{头像设置}：将头像图片放入 Image 文件夹，修改 HTML 中的图片路径
    \item \textbf{学习笔记集成}：将微分几何学习笔记（main.pdf）链接到主页项目展示区
...
\begin{itemize}
    ...
    \item \textbf{图片格式问题}：GPU 并行示意图原为 PNG 格式，在网页中无法正常显示，后转换为 JPG 格式解决
...
\begin{itemize}
    \item \textbf{网页框架搭建}：AI 辅助生成了主页、博客列表页、博客内容页的基本 HTML 结构与 CSS 样式
    \item \textbf{内容填充与修改}：根据用户提供的"主页内容.txt"，AI 辅助完成了个人信息、头像、研究方向等内容的修改
...
\begin{itemize}
    \item \textbf{内容个性化}：AI 生成的初始内容中的人物名称（张三）被替换为真实姓名（田天威），简介文字也根据个人实际情况进行了修改
    \item \textbf{主题选择}：博客主题从最初的线性代数调整为计算机图形学中的光线追踪内容，更符合个人兴趣与研究方向
    \item \textbf{图片替换}：删除了 AI 生成的线性代数博客，替换为光线追踪专题文章，并增补了路径追踪与俄罗斯轮盘赌的内容
